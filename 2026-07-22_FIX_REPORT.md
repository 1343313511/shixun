...
**问题：** `profile` 路由接受 `user_id` 参数但不校验当前用户有无权限访问其他用户的数据。验证码绕过漏洞——攻击者可指定任意 `user_id` 查看其他用户的个人资料。

`recharge` 路由同样接受 `user_id` 作为表单参数，没有校验只能给自己充值。理论上攻击者可以通过修改表单 `user_id` 给任意账户充值。
